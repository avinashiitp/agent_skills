...
Level 7 — MCP Server
=====================
Exposes 8 tools via MCP protocol. Identical to Level 6 server.
Skills live on the CLIENT side — this server only holds tool logic.
...
mcp = FastMCP("Level 7 MCP Server")
...
def calculate(expression: str) -> str:
    ...
    Supports: +, -, *, /, **, sum(), abs(), round(), min(), max()
    Example: '2**16', '(100 * 3.14159) / 2', 'sum([1,2,3,4,5])'
    """
    try:
        allowed_names = {
            "__builtins__": {},
...
def convert_units(value: float, from_unit: str, to_unit: str) -> str:
    ...
    Units must be lowercase: km, miles, kg, lbs, celsius, fahrenheit, liters, gallons.
    """
    frm = from_unit.lower()
    to = to_unit.lower()
    conversions = {
...
def analyze_text(text: str) -> str:
    """
    Analyze a piece of text: word count, sentence count, character count.
    """
...
def get_weather(city: str) -> str:
    ...
    Returns temperature in Celsius and Fahrenheit, conditions, humidity, feels-like.
    """
    try:
        resp = requests.get(
            f"https://wttr.in/{city}?format=j1",
            headers={"User-Agent": "Level7-Agent/1.0"},
            timeout=8,
        )
        data = resp.json()
...
def get_country_info(country: str) -> str:
    """
    Get facts about a country: capital, population, region, area in km², currencies.
    """
    try:
        resp = requests.get(
            f"https://restcountries.com/v3.1/name/{country}"
            f"?fields=name,capital,population,region,area,currencies",
            timeout=8,
        )
        data = resp.json()[0]
...
def get_random_fact() -> str:
    """
    Fetch a random interesting fact from the internet.
    """
    try:
        resp = requests.get(
...
def get_github_user(username: str) -> str:
    """
    Look up a GitHub user profile: name, bio, public repos, followers, location, company.
    """
    try:
        resp = requests.get(
...
def define_word(word: str) -> str:
    ...
    Returns up to 2 parts of speech with up to 2 definitions each.
    """
    try:
        resp = requests.get(
